...
class CapacityReport:
    ...
    def run_assigned_moves(self, start_study_date: 'MM/DD/YYYY', end_study_date: 'MM/DD/YYYY'):
        ...
        for a_step_flow in step_flow_combos:
            ...
            list_of_outs_dates = [x + timedelta(days = int(y * interval_plan_period())) for x,y in zip(list_of_moves_date, list_of_CT)]
            # easy move:  list_of_moves = [self.assigned_moves.at[a_step_flow, 'FLOW'] for x in list_of_moves_date]
            # not so easy move:
            list_of_moves = [sum([self.assigned_moves.at[x, self.stepMatrix.at[a_step_flow, 'FLOW'] + '.' + subflow] for subflow in self.stepMatrix.at[a_step_flow, 'SUB_FLOW'].split(',')]) for x in list_of_outs_dates]
            #list_of_moves = []
            #for a_move_date in list_of_outs_dates:
            #    a_move = sum([self.assigned_moves.at[a_move_date, self.stepMatrix.at[a_step_flow, 'FLOW'] + subflow] for subflow in ['.FF','.PCMOS']])
